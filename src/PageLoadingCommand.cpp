...
void PageLoadingCommand::start() {
  m_manager->logger() << QTime::currentTime().toString() << ": Started" << m_command->toString();
...
void PageLoadingCommand::pendingLoadFinished(bool success) {
  ...
  if (m_pageLoadingFromCommand) {
    ...
    if (m_pendingResponse) {
      m_manager->logger() << QTime::currentTime().toString() << ": Page load from command finished";
      if (m_pageSuccess) {
...
void PageLoadingCommand::pageLoadingFromCommand() {
  m_manager->logger() << QTime::currentTime().toString() << ": " << m_command->toString() << "started page load";
  m_pageLoadingFromCommand = true;
}

void PageLoadingCommand::commandFinished(Response *response) {
  disconnect(m_manager, SIGNAL(loadStarted()), this, SLOT(pageLoadingFromCommand()));
  m_manager->logger() << QTime::currentTime().toString() << ": Finished" << m_command->toString();
